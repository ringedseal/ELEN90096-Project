...
#pragma HLS PIPELINE II=1
                        for (int tw = 0; tw < tw_eff; ++tw) {
                            output_ftmap[k_channel + tn]
                                        [i_height + th]
                                        [j_width  + tw] = tile_buf[tn][th][tw];
                        }
                    }
                }
            }
...
void conv1_tile(
	    ...
	    param_t  conv1_weights[N1][N0][F1][F1],
	    param_t  conv1_biases[N1],
	    int      k_channel,   // which output-channel block (multiple of Tn1)
	    int      i_height,    // top-left pixel row of this tile
	    int      j_width,     // top-left pixel col of this tile
	    int      tn_eff,      // effective output channels in this block (<=Tn1, handles boundary)
	    int      th_eff,      // effective tile height (<=Th1, handles bottom edge)
	    int      tw_eff,      // effective tile width  (<=Tw1, handles right edge)
...
void conv1_tile_fused(
    ftmap_t  input_ftmap[N0][H][W],
    param_t  conv1_weights[N1][N0][F1][F1],
    param_t  conv1_biases[N1],
    int      tile_row,
    int      tile_col,
    ftmap_t  conv1_tile_out[N1][Th3+F3-1][Tw3+F3-1]
){

    const int pad3 = (F3 - 1) / 2;


    const int tile_y0 = tile_row * Th3;
    const int tile_x0 = tile_col * Tw3;

    const int tile_halo_h = Th3 + F3 - 1;
    const int tile_halo_w = Tw3 + F3 - 1;


    CONV1_OC_LOOP:
    for (int oc = 0; oc < N1; ++oc) {
    CONV1_LY_LOOP:
        for (int ly = 0; ly < tile_halo_h; ++ly) {
        CONV1_LX_LOOP:
            for (int lx = 0; lx < tile_halo_w; ++lx) {
            #pragma HLS PIPELINE II=1


                int global_y = tile_y0 + ly - pad3;
                int global_x = tile_x0 + lx - pad3;


                if (global_y < 0)       global_y = 0;
                else if (global_y >= H) global_y = H-1;
                if (global_x < 0)       global_x = 0;
                else if (global_x >= W) global_x = W-1;


                ftmap_t acc = conv1_biases[oc];
                ...
                CONV1_KY_LOOP:
                    ...
                    CONV1_KX_LOOP:
                        for (int kx = 0; kx < F1; ++kx) {
                            ...
                            int iy = global_y + ky - (F1/2);
                            int ix = global_x + kx - (F1/2);
                            ...
                            acc += conv1_weights[oc][ic][ky][kx] * input_ftmap[ic][iy][ix];
                        }
                    }
                }
