...
void srcnn_prev(ftmap_t input_ftmap[N0][H][W],
               param_t conv1_weights[N1][N0][F1][F1],
               param_t conv1_biases[N1],
               param_t conv2_weights[N2][N1][F2][F2],
               param_t conv2_biases[N2],
...
#pragma HLS DEPENDENCE variable=conv1_out intra false inter false
...
void conv2_pointwise(
    ftmap_t in[N1][Th3+F3-1][Tw3+F3-1],
	param_t weights[N2][N1][1][1],
	param_t biases[N2],
    ftmap_t out[N2][Th3+F3-1][Tw3+F3-1])
{
#pragma HLS PIPELINE II=1
    for (int oc = 0; oc < N2; ++oc)
        for (int y = 0; y < Th3+F3-1; ++y)
            for (int x = 0; x < Tw3+F3-1; ++x) {
                ftmap_t acc = biases[oc];
                for (int ic = 0; ic < N1; ++ic)
                    acc += weights[oc][ic][0][0] * in[ic][y][x];
                out[oc][y][x] = relu(acc);
            }
}

void conv3_tile_local(
    ftmap_t  in_tile[N2][Th3+F3-1][Tw3+F3-1],
    param_t  weights[N3][N2][F3][F3],
    param_t  biases[N3],
    ftmap_t  out_tile_local[N3][Th3][Tw3]
){
#pragma HLS INLINE off
...
#pragma HLS UNROLL factor=4
                    for (int ky = 0; ky < F3; ++ky)
                        for (int kx = 0; kx < F3; ++kx)
                            acc += weights[oc][ic][ky][kx] * in_tile[ic][y+ky][x+kx];
                }
                out_tile_local[oc][y][x] = acc;
            }
        }
    }
...
void process_tile(
	...
	param_t    conv1_weights[N1][N0][F1][F1],
	param_t    conv1_biases[N1],
	param_t    conv2_weights[N2][N1][1][1],
	param_t    conv2_biases[N2],
	param_t    conv3_weights[N3][N2][F3][F3],
	param_t    conv3_biases[N3],
    ftmap_t output_tile[N3][Th3][Tw3],
    int tile_row, int tile_col)
{
#pragma HLS DATAFLOW

    static ftmap_t conv1_tile_out[N1][Th3+F3-1][Tw3+F3-1];
    static ftmap_t conv2_tile_out[N2][Th3+F3-1][Tw3+F3-1];

    conv1_tile_fused(input_ftmap, conv1_weights, conv1_biases, tile_row, tile_col, conv1_tile_out);
    conv2_pointwise(conv1_tile_out, conv2_weights, conv2_biases,
                    conv2_tile_out);
    conv3_tile_local(conv2_tile_out, conv3_weights, conv3_biases, output_tile);
}
...
void srcnn(ftmap_t input_ftmap[N0][H][W],
            ...
            ftmap_t output_ftmap[N3][H][W])
{
    const int tiles_H = (H + Th3 - 1) / Th3;
    const int tiles_W = (W + Tw3 - 1) / Tw3;

    for (int ti = 0; ti < tiles_H; ++ti){
        for (int tj = 0; tj < tiles_W; ++tj) {
#pragma HLS LOOP_FLATTEN off
#pragma HLS PIPELINE II=1
            ftmap_t out_tile[N3][Th3][Tw3];
            process_tile(input_ftmap, conv1_weights, conv1_biases, conv2_weights, conv2_biases, conv3_weights, conv3_biases, out_tile, ti, tj);
            ...
            for (int n = 0; n < N3; ++n){
                ...
                for (int y = 0; y < Th3; ++y){
                    ...
                    for (int x = 0; x < Tw3; ++x){
                        ...
                        int gx = out_x0 + x;
                        if (gx >= W) break;
                    	output_ftmap[n][ti*Th3 + y][tj*Tw3 + x] = out_tile[n][y][x];
                    }
                }
